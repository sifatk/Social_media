...
<body>
    ...
    <nav class="nav-wrapper deep-orange">
        ...
        <ul class="right">
            ...
            <li>
                <!-- Logout -->
                <a href="backend_files/logout_backend.php"><i class="material-icons" id="log_out">exit_to_app</i></a>
    ...
    <?php
    $sql_to_fetch_notification_current = "SELECT notification.Notification_Timestamp, notification.Notification_Text FROM notification JOIN user_information ON notification.User_ID = user_information.User_ID WHERE user_information.Email LIKE '$current'";

    $result_to_fetch_notification_current = mysqli_query($conn, $sql_to_fetch_notification_current);

    if (mysqli_num_rows($result_to_fetch_notification_current) > 0) {
        while ($row = mysqli_fetch_row($result_to_fetch_notification_current)) {
    ?>
            <p><?= $row[1] ?></p>
    <?php
        }
